...
def find_bad_hk_durations_no_ts(parameters):
	...
	for file in os.listdir('.'):
		...
		if file.endswith('.hk'):
			...
			hk_filedata = np.genfromtxt(file, delimiter="\t", skip_header=1, usecols=(1,3,4)) #timestamp since midnight local time, measured sample flow, Yag power

			#convert the time since midnight data to unix timestamps in UTC
			hk_filedata[:,0] = hk_filedata[:,0] + hk_timestamp + timezone
