...
start = datetime(2009,7,4)  #2009 - 20090628  2010 - 20100610   2012 - 20100405
end =   datetime(2011,6,1)  #2009 - 20090816  2010 - 20100726   2012 - 20100601
timestep = 24 
...
while start <= end:
	...
	cursor.execute('''(SELECT 
		...
		(UNIX_start,UNIX_end,101,1,3,5,10))	
		#
	...
	for row in hrly_data:
		...
		if (4 <= date_time.hour < 16): #nighttime data only
			if (fire_time1[0] <= date_time < fire_time1[1]) or (fire_time2[0] <= date_time < fire_time2[1]): #skip bb times
				#print 'fire!',date_time 
				continue
			if sp2_vol == 0:
				#print 'no_vol'
				continue
				
			sp2_conc = sp2_mass_conc*corr_factor_for_massdistr #ng/m3
...
ax1.hist(sp2,bins=40,range = (0,300),color = 'b')
ax1.set_xlim(0,300)
#ax1.set_ylim(0,60)
ax1.set_ylabel('freq')
...
ax2.hist(gcd,bins=40,range = (0,300),color = 'r')
ax2.set_xlim(0,280)
#ax2.set_ylim(0,60)
ax2.set_ylabel('freq')
