...
add_data = ('INSERT INTO polar6_uhsas_rbc_binned_data_altcalib'
...
for UHSAS_file in os.listdir('.'):	
	if UHSAS_file.endswith('.ict'):
		with open(UHSAS_file, 'r') as f:
			...
			for line in f:
				...
				if data_start == True:
					...
					cursor.execute(('SELECT UNIX_UTC_ts FROM polar6_coating_2015 WHERE UNIX_UTC_ts < %s AND particle_type = %s and rBC_mass_fg >=%s and rBC_mass_fg <=%s order by UNIX_UTC_ts desc limit 1'),(time_max, 'incand', min_rBC_mass,max_rBC_mass))
					...
					cursor.execute(('SELECT UNIX_UTC_ts FROM polar6_coating_2015 WHERE UNIX_UTC_ts < %s AND particle_type = %s and rBC_mass_fg >=%s and rBC_mass_fg <=%s order by UNIX_UTC_ts desc limit 1'),(time_min, 'incand', min_rBC_mass,max_rBC_mass))
					...
					if prev_particle_data == []:
						...
						cursor.execute(('SELECT UNIX_UTC_ts FROM polar6_coating_2015 WHERE UNIX_UTC_ts >= %s AND particle_type = %s and rBC_mass_fg >=%s and rBC_mass_fg <=%s order by UNIX_UTC_ts limit 1'),(time_min, 'incand', min_rBC_mass,max_rBC_mass))
					...
					for bin_number in range(0,len(bin_dict)):
						...
						cursor.execute(('SELECT count(*) from polar6_coating_2015 where UNIX_UTC_ts >= %s and UNIX_UTC_ts < %s and particle_type = %s and (POW(rBC_mass_fg,(1/3.0))*101.994391398+2*coat_thickness_nm_jancalib) >=%s and (POW(rBC_mass_fg,(1/3.0))*101.994391398+2*coat_thickness_nm_jancalib) <=%s'),
							(time_min,time_max, 'incand', bin_LL,bin_UL))
						core_plus_coating_count = cursor.fetchall()[0][0]
						#get core only data 
						cursor.execute(('SELECT rBC_mass_fg, coat_thickness_nm_jancalib from polar6_coating_2015 where UNIX_UTC_ts >= %s and UNIX_UTC_ts < %s and particle_type = %s and (POW(rBC_mass_fg,(1/3.0))*101.994391398) >=%s and (POW(rBC_mass_fg,(1/3.0))*101.994391398) <=%s'),
							(time_min,time_max, 'incand', bin_LL,bin_UL))
						core_only_data = cursor.fetchall()		
						#get nonincand data 
						cursor.execute(('SELECT count(*) from polar6_calibration_data where UNIX_UTC_ts >= %s and UNIX_UTC_ts < %s and particle_type = %s and particle_dia >=%s and particle_dia <=%s'),
							(time_min,time_max, 'nonincand', bin_LL,bin_UL))
						nonincand_count = cursor.fetchall()[0][0]	
						
						
						#### UHSAS data
						UHSAS_norm_number_str = newline[bin_number+4].rstrip(',')
						UHSAS_norm_number = float(UHSAS_norm_number_str)	#this is dN/dLogD per sccm	
						...
						if UHSAS_norm_number < 0 or UHSAS_norm_number_str == 'nan' : #-9999 is missing data and any other negative is a data problem
							#print UNIX_time_stamp, 'no UHSAS data'
							UHSAS_number = None
							core_plus_coating_number_conc = None
							core_only_number_conc = None
							nonincand_number_conc = None
						elif TandP_data == []:
							...
							if no_prev_particle == True:  #in this case need to ignore first particle (but don't want negative if the count is zero)
								...
								if nonincand_count > 0:
									nonincand_number_conc = (nonincand_count-1)*correction_factor_for_STP/interval_sampled_volume  #dN/sccm
								else:
									nonincand_number_conc = 0
						...
						if UHSAS_number != None and core_plus_coating_number_conc != None and core_only_number_conc != None and nonincand_number_conc != None:
							binned_data = {
							'UNIX_UTC_ts': UNIX_time_stamp,
							'binLL':  bin_LL,
							'binUL':  bin_UL,
