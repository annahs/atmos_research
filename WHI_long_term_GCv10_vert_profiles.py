...
molar_mass_OC = 12.0107 #in g/mol
molar_mass_SO4 = 96.0626 #in g/mol
ng_per_g = 10**9
ug_per_g = 10**6
	...
	for file in os.listdir(data_dir):
		if file.endswith('.hdf'): 
			...
			if start_hour <= file_hour < end_hour:  #ignore any times not in the 2000-0800 window 
				...
				hydrophilic_OC = hdf_file.select('IJ-AVG-$::OCPI')
				hydrophobic_OC = hdf_file.select('IJ-AVG-$::OCPO')
				i=0
				...
				for level in range(1,47):
					...
					SO4_ugm3 = SO4_ppbv * molar_mass_SO4 *ug_per_g*GEOS_Chem_factor*(101325/(R*273))
					total_OC_ppbv = hydrophilic_OC[level,lat,lon] + hydrophobic_OC[level,lat,lon]
					OC_conc_ugm3 = total_OC_ppbv*molar_mass_OC*ug_per_g*GEOS_Chem_factor*(101325/(R*273)) 
					OM_conc_ugm3 = 1.8*OC_conc_ugm3 #see http://wiki.seas.harvard.edu/geos-chem/index.php/Particulate_matter_in_GEOS-Chem
							
					scenario[cluster][level].append([pressure,OM_conc_ugm3,GC_datetime])
					scenario['all'][level].append([pressure,OM_conc_ugm3,GC_datetime])
...
for cluster in cluster_list:	
	...
	if i == 3:
		...
		axs[i].set_xlabel('[OM mass](ug/m3 - STP)')
	if i == 4:
