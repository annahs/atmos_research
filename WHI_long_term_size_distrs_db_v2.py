...
def calcuate_VED(bbhg_incand_pk_amp, year):
...
def assemble_interval_data(retrieved_records):
	...
	for row in retrieved_records:
		...
		sample_flow = 120.
		year = datetime.utcfromtimestamp(ind_start_time).year
		if sample_flow == None:
			continue
		...
		VED = calcuate_VED(bbhg_incand_pk_amp, year)
		ved_list.append(VED)
	...
	for dia in raw_dia_list:	
		for point in bin_dict:
			...
			if (LL_bin <= dia < UL_bin):
				...
				#2012-06-07 18:14:39	1.0
				#2012-06-11 19:44:25	5.0
				#2012-06-11 19:46:46	10.0
				#2012-07-03 16:33:10	5.0
				#2012-07-08 15:04:45	10.0
				#2012-07-12 18:34:01	5.0
...
def	write_files(core_bin_mids,core_norm_mass_concs,core_norm_numb_concs,fit_bin_mids,mass_fit_points,numb_fit_points,ratio_uncertainty):
...
def plot_distrs(data_to_fit_bins,data_to_fit_mass_concs,data_to_fit_number_concs,mass_fit_params,number_fit_params,fractions_list,instr_id,total_vol):
	...
	if write_to_file == True:
		write_files(core_bin_mids,core_norm_mass_concs,core_norm_numb_concs,fit_bin_mids,mass_fit_points,numb_fit_points,ratio_uncertainty)
...
cursor.execute('''(SELECT 
						sp2.UNIX_UTC_ts_int_start,
						sp2.UNIX_UTC_ts_int_end,
						sp2.BB_incand_HG,
						cl.cluster_number
					FROM
						whi_sp2_particle_data sp2 
							JOIN
						whi_ft_cluster_times_2009to2012 cl on sp2.UNIX_UTC_ts_int_start BETWEEN cl.cluster_start_time AND cl.cluster_end_time
							JOIN
						whi_hk_data hk on sp2.hk_id = hk.id   
							left JOIN
						whi_spike_times_2009to2012 spk on sp2.UNIX_UTC_ts_int_start BETWEEN spk.spike_start_UTC AND spk.spike_end_UTC
					WHERE
						cl.cluster_number = %s OR cl.cluster_number = %s
						AND
						spk.id is null 
						AND
						hk.sample_flow BETWEEN 118 AND 123 
						AND
						hk.yag_power   BETWEEN 4 AND 7;)''',
				(4,4))
